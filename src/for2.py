...
class TurtleBotGTG:
    ...
    def goal_movement(self):
        # Ensure both positions are available before computing relative position
        # Get user input for teleportation
        waffle_x = float(input("Enter x position for waffle: "))
        waffle_y = float(input("Enter y position for waffle: "))
        obstacle_x = float(input("Enter x position for obstacle: "))
        obstacle_y = float(input("Enter y position for obstacle: "))

        # Set the initial positions
        waffle_position = Point(waffle_x, waffle_y, 0)
        obstacle_position = Point(obstacle_x, obstacle_y, 0)
        orientation = Quaternion(0, 0, 0, 1)  # No rotation

        success, message = self.set_model_state('turtlebot3_waffle', waffle_position, orientation)
        if success:
            rospy.loginfo("Waffle robot teleported successfully")
        else:
            rospy.logerr("Failed to teleport waffle robot: %s" % message)

        success, message = self.set_model_state('turtlebot3_obstacle', obstacle_position, orientation)
        if success:
            rospy.loginfo("Obstacle robot teleported successfully")
        else:
            rospy.logerr("Failed to teleport obstacle robot: %s" % message)    
