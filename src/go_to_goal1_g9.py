...
class TurtleBotGTG:
    ...
    def goal_movement(self):
        ...
        while not rospy.is_shutdown():
            ...
            lamdaa_radians = np.arcsin(dmin / do)  
            ...
            if magnitude_rel_vel == 0 or magnitude_rel_pos == 0:
                ...
                cos_theta = 0  # Set a default value or handle it as required
            else:
                # Calculate the cosine of the angle
                cos_theta = dot_product / (magnitude_rel_vel * magnitude_rel_pos)

            print("cosine_theta_rad:", cos_theta)

            # Calculate alpha_radians safely
            alphaa_radians = np.arccos(cos_theta)

            alphaa = math.degrees(alphaa_radians)
            print("alphaa:", alphaa)
            ...
            # Steer commands based on alphaa, lambdaa and psi_waffle
                if ((psi_waffle) < (alphaa_plus_lambdaa)) and ((psi_waffle) > (alphaa)):
                ...
                elif ((psi_waffle) > (alphaa_minus_lambdaa)) and ((psi_waffle) < (alphaa)):
