...
class gnc_api:
    ...
    def enu_2_local(self):
        ...
        # Convert from ENU to the local frame
        current_pos_local.x = x * cos(radians(self.local_offset_g)) - y * sin(radians(self.local_offset_g))
        current_pos_local.y = x * sin(radians(self.local_offset_g)) + y * cos(radians(self.local_offset_g))
    ...
    #     # Adjust for NWU to NED conversion
    #     current_pos_local.x = y
    #     current_pos_local.y = -x
    #     current_pos_local.z = z
    ...
    def set_destination(self, x, y, z, psi):
        ...
        theta = radians(self.local_offset_g)
        ...
        rospy.loginfo(CRED2 + f"Correction Vector x:{self.correction_vector_g.position.x} y:{self.correction_vector_g.position.y} z:{self.correction_vector_g.position.z}" + CEND)
        rospy.loginfo(CRED2 + f"Local Offset x:{self.local_offset_pose_g.x} y:{self.local_offset_pose_g.y} z:{self.local_offset_pose_g.z}" + CEND)

        # Convert desired local frame coordinates to ENU frame      
        # theta = radians(self.local_offset_g)
        # Xlocal = 0 # x * cos(theta) - y * sin(theta)
        # Ylocal = 0 # x * sin(theta) + y * cos(theta)
        # Zlocal = 0 # z
        ...
        # x = self.correction_vector_g.position.x + self.local_offset_pose_g.x
        # y = self.correction_vector_g.position.y + self.local_offset_pose_g.y
        # z = self.correction_vector_g.position.z + self.local_offset_pose_g.z
    ...
    # def set_destination(self, x, y, z, psi):
    #     # Adjust heading for NWU to NED conversion
    #     psi_ned = psi - 90.0

    #     self.set_heading(psi_ned)

    #     # Rest of your set_destination method remains unchanged
    #     theta = radians((self.correction_heading_g + self.local_offset_g - 90))
    ...
    #     rospy.loginfo(
    #         "Destination set to x:{} y:{} z:{} in Ardupilot's NED frame".format(x, y, z))
    ...
    # def set_destination(self, x, y, z, psi):
    #     # Convert to NWU frame directly without heading adjustment
    #     self.set_heading(psi)

    #     # Set the waypoint in NWU frame
    #     self.waypoint_g.pose.position = Point(x, y, z)
    ...
    #     rospy.loginfo(
    #         "Destination set to x:{} y:{} z:{} in Gazebo's NWU frame".format(x, y, z))
