...
class DroneController:
    ...
    def compute_los_euler_angles(self):
        x = self.target_pos[0] - self.pursuer_pos[0]
        y = self.target_pos[1] - self.pursuer_pos[1]
        z = self.target_pos[2] - self.pursuer_pos[2]
        # print("Pursuer: ", self.pursuer_pos)
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            if self.pursuer_pos and self.target_pos:
                # Get orientation from target position
                roll, pitch, yaw = self.compute_los_euler_angles()
                orientation = self.quaternion_from_euler(roll, pitch, yaw)
                # print("orientation: ", orientation)
                # Position of the cone base is the position of the target
