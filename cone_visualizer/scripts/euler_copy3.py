...
if __name__ == "__main__":
    ...
    # cone_height = 1.0 
    cone_radius = 0.5  # Adjust the radius 
    color = (1.0, 0.0, 0.0, 1.0)  # RGBA

    rate = rospy.Rate(10)
    while not rospy.is_shutdown():
        
        cone_marker = create_cone_marker(frame_id, 0, position, orientation, length, cone_radius, color)
         
        cap_marker = create_spherical_cap_marker(frame_id, 1, position, orientation, length, cone_radius, color)
