...
def calculate_geometry():
    """
    Calculate the slant height, cap height, and cap radius of a spherical cap.
    """
...
def create_cone_marker(frame_id, marker_id, position, orientation, height, radius):
    marker = Marker()
    ...
    for i in range(n_segments):
        theta = i * delta_theta
        p1 = Point(0, 0, 0)
        p2 = Point(height, radius * math.cos(theta), radius * math.sin(theta))
        p3 = Point(height, radius * math.cos(theta + delta_theta), radius * math.sin(theta + delta_theta))
...
def create_spherical_cap_marker(frame_id, marker_id, position, orientation):
    marker = Marker()
    marker.header.frame_id = frame_id
    marker.header.stamp = rospy.Time.now()
    ...
    slant_height, cap_height, cap_radius, height,radius = calculate_geometry()
    ...
    marker.pose.orientation = orientation
    marker.scale.x = 0.01
    marker.color.r, marker.color.g, marker.color.b, marker.color.a = (1.0, 0, 0, 0.5)
    return marker

def calculate_orientation_and_height(position, target_vector):
...
def main():
    ...
    orientation, cone_height = calculate_orientation_and_height(position, target_vector)

    rate = rospy.Rate(2)
    while not rospy.is_shutdown():
        cone_marker = create_cone_marker(frame_id, 0, position, orientation, cone_height, radius)
        cap_marker = create_spherical_cap_marker(frame_id, 1, position, orientation)
