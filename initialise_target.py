...
class DroneController:
    def __init__(self):
        ...
        rospy.Subscriber('/target/mavros/state', State, self.state_callback)
        rospy.Subscriber('/target/mavros/local_position/pose', PoseStamped, self.pose_callback)

        # Create services for arming and setting mode
        self.arming_client = rospy.ServiceProxy('/target/mavros/cmd/arming', CommandBool)
        self.set_mode_client = rospy.ServiceProxy('/target/mavros/set_mode', SetMode)

        # Create a publisher for the local position goal
        self.pose_goal_pub = rospy.Publisher('/target/mavros/setpoint_position/local', PoseStamped, queue_size=10)
